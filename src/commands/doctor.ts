...
function logProviderStatus(
  ...
  if (cliInstalled && enabled) {
    ...
    if (!status.hooks.ok || !status.agents.ok || !status.skills.ok || !status.installedSkills.ok) {
      hasIssues = true;
    }
...
function isFullyInstalled(status: InstallationStatus): boolean {
  return status.hooks.ok && status.agents.ok && status.skills.ok && status.installedSkills.ok;
}
